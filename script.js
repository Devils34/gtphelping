...
function openAreYouSure(item) {
  ...
  var base = '<div id="sure_gradient" class="slide-right">' +
    ...
    '    <p>VOUS ETES SUR ?</p>' +
    '   </div>' +
    '<div id="sure_confirm" onclick="buyItem(\'' + item + '\')" class="ripple">CONFIRMER</div>' +
    '<div id="sure_cancel" onclick="cancelAreYouSure()" class="ripple">SUPPRIMER</div>' +
    '<div>';
...
function winningChoose(item, t) {
  ...
  var base = '<div id="sure_gradient" class="slide-right">' +
    ...
    '    <p>CHOISIR</p>' +
    '   </div>' +
    '<div id="sure_redeem" onclick="redeem(\'' + item + '\')" class="ripple">RECUPERER</div>' +
    '<div id="sure_exchange" onclick="exchange(\'' + item + '\')" class="ripple">ECHANGER</div>' +
    '<div id="sure_close" onclick="cancelAreYouSure()" class="ripple">FERMER</div>' +
    '<div>';
...
function multiplyValue(x) {
  $("#input_credits").val(parseInt($("#input_credits").val()) * x);
}
...
function openCaseOpening() {
  ...
  var base = '<div class="clearfix borderbox slide-right" id="creditmenu"><!-- column -->' +
    ...
    '<div id="category" class="ripple" onclick="openWinnings()">GAINS</div>' +
  ...
  base = base + '    </ul>' +
    ...
    '  </div>' +
    '<div id="bettingoptions">OPTIONS DE MISES</div>' +
    '<input id="input_credits">' +
    '     <img class="block" id="u105_img" src="img/creditcoin.png" alt="" width="42" height="42"/>' +
    '</input> ' +
    '  <div class="button ripple" id="startSpinner">OUVRIR</div>' +
    ...
    '  <div class="button5 ripple" onclick="clearValue()">VIDER</div>' +
    '</div>' +
  ...
  startSpinnerBtn.addEventListener("click", (e) => {
    ...
    console.log('ImportChance: ' + (odds['import'] - ((betAmount / highestExchange)) / 3))
    console.log('ImportChance: ' + (odds['superrare'] - ((betAmount / highestExchange)) / 3))
    //console.log(lowestExchange + ' ' + highestExchange);
    for (i = 0; i < 100; i++) {


      if (Math.random() > (odds['import'] - ((betAmount / highestExchange)) / 3)) {
      ...
      } else if (Math.random() > (odds['superrare'] - ((betAmount / highestExchange)) / 3)) {
      ...
      } else if (Math.random() > (odds['rare'] - ((betAmount / highestExchange)) / 3)) {
      ...
      } else if (Math.random() > (odds['common'] - ((betAmount / highestExchange)) / 3)) {
      ...
      } else if (Math.random() > (odds['supercommon'] - ((betAmount / highestExchange)) / 3)) {
...
function itemWon(win) {
  ...
  setTimeout(function () {
    ...
    var base = '<div id="win_gradient" class="slide-right">' +
      ...
      '<div id="win_confirm" onclick="closeItemWon()" class="ripple">FERMER</div>' +
      '<div>';
...
function openBuy() {
  ...
  var base = '<div class="clearfix borderbox slide-right" id="creditmenu"><!-- column -->' +
    ...
    '<div id="buy_button" onclick="openWeb()" class="ripple">BOUTIQUE</div>' +
  ...
  if(winnings !=null && winnings !=undefined){
    ...
    for (const [key, value] of Object.entries(winnings)) {
      ...
      for (i = 0; i < winnings[key]; i++) {
        ...
        if(caseItems[key] !=null){
          if(caseItems[key].exchange !=null && caseItems[key].exchange !=undefined ){ 
